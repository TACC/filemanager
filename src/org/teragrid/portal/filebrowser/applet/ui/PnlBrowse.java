...
//      else if (mnuFileVersionHistory == e.getSource()) { // uncomment when adding share support
//      	// show version history panel
//      	LogManager.debug("Display version history panel");
//      	String path = getCurrentDir() + separator + file.getName();
//      	DlgVersionHistory dlgHistory;
//		try {
//			dlgHistory = new DlgVersionHistory(AppMain.getFrame(),(TGShareFileInfo)file,tBrowse.ftpSrvConn.getVersionHistory(path));
//			Version previousVersion = dlgHistory.getSelectedVersion();
//	      	if (previousVersion != null) {
//	      		tBrowse.cmdAdd("Rollback", path, previousVersion);
//	      	}
//		} catch (ServerException e1) {
//			LogManager.error("Failed to retrieve version history for " + path,e1);
//			AppMain.Error(this,"Failed to retrieve version history for " + path);
//		}
//      	
//      	return;
//      }
		...
		if (clickedRow != -1) {
			...
			if (SwingUtilities.isRightMouseButton(e)) {
				...
				if (tBrowse.ftpServer.type == FTPType.XSHARE) {
					...
					mnuFileMetadataShow.setVisible(false);
				} else if (tBrowse.ftpServer.type == FTPType.IRODS) { 
