...
import org.teragrid.portal.filebrowser.applet.file.FileInfoFactory;
	...
	private IRODSAccount getAccount() 
	{
		try
		{
			if (StringUtils.isEmpty(this.password) && StringUtils.isEmpty(this.username)) {
				return IRODSAccount.instance(this.host, this.port, 
					new GlobusGSSCredentialImpl(AppMain.defaultCredential, GSSCredential.INITIATE_AND_ACCEPT));
			} else {
