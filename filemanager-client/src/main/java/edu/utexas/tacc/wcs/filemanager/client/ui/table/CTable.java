...
import edu.utexas.tacc.wcs.filemanager.client.ui.GridFileView;
import edu.utexas.tacc.wcs.filemanager.client.ui.IconData;
import edu.utexas.tacc.wcs.filemanager.client.ui.PnlBrowse;
import edu.utexas.tacc.wcs.filemanager.client.ui.TransferProxy;
...
public class CTable extends StripedTable implements GridFileView {
    ...
    public CTable(final PnlBrowse frmParent, boolean bRemoteMode) {
        super();
...
//            fileList.add((FileInfo)((IconData)((DetailListModel)getModel()).getFileList().get(nRows[i])).getObject());
    ...
    public List<FileInfo> getSelectedFiles() {
    	...
    	for(int row: getSelectedRows()) {
    		files.add((FileInfo)((DetailListModel)getModel()).getFile(row));
    	}
    	return files;
    }
